...
/**
 * Splits a wire fallback into the provider and model the sheet's two selects need. Unpinned
 * fallbacks arrive as the bare "provider/model" string; pinned ones carry the same combined
 * string in their `model` field.
 */
export function normalizeFallback(fallback: RoutingFallbackWire): RoutingFallbackFormData {
	const combined = (typeof fallback === "string" ? fallback : (fallback.model ?? "")).trim();
	const keyId = typeof fallback === "string" ? "" : (fallback.key_id ?? "");
	const separator = combined.indexOf("/");
	if (separator === -1) {
		return { provider: combined, model: "", key_id: keyId };
	}
	return {
		provider: combined.slice(0, separator),
		model: combined.slice(separator + 1),
		key_id: keyId,
	};
}

/**
 * Joins a fallback back onto the wire, as the bare string unless it pins a key. Sending the object
...
export function denormalizeFallback(fallback: Partial<RoutingFallbackFormData>): RoutingFallbackWire {
	const combined = `${(fallback.provider ?? "").trim()}/${(fallback.model ?? "").trim()}`;
	const keyId = (fallback.key_id ?? "").trim();
	return keyId ? { model: combined, key_id: keyId } : combined;
}
