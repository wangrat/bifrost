...
| Field | Required | Description |
|-------|----------|-------------|
| `model` | Yes | The same combined `"provider/model"` form the string carries. Use `"provider/"` to keep the incoming request's model |
...
"fallbacks": [
  ...
  { "model": "vertex/", "provider_key_name": "vertex-prod" }
]
```
