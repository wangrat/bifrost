...
- **fallbacks** (array, optional): Ordered fallback chain, tried after every target fails. Each entry is either the string `"provider/model"` (use `"provider/"` to keep the incoming model), or an object:
  - `model` (string, required): The same combined `"provider/model"` form the string carries, and the same form the request body's `model` field uses
...
```json
{
  ...
  "fallbacks": [
    "azure/gpt-4o",
    { "model": "vertex/gemini-2.5-pro", "key_id": "8f3c1d2e-..." }
  ]
}
```
