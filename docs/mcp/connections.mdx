...
flowchart LR
    ...
    D -->|automatic reconnect succeeds| C
    D -->|credential dead, no silent recovery| E[needs_reauth]
    C -->|credential dead| E
    E -->|admin reauthorizes| B
```

- **Connect** happens once at `AddClient` (boot, or client creation) and again on every reconnect. For `http` and `sse`, reconnects are [make-before-break](./gateway#reconnection-behavior): the old connection keeps serving until the new one is ready, so token rotation never causes downtime. `stdio` and in-process clients close first instead, because two subprocesses would double resource use and clash over anything held exclusively.
- **`unstable`** is purely informational: a network blip doesn't stop tool calls, and the client recovers with no human involvement. Two things heal it. A failed periodic check reconnects the connection it just failed to probe, so a session the server expired, a dropped SSE stream, or a `stdio` server whose process exited is replaced on the next check rather than probed forever. And a tool call that fails because the session is gone triggers the same reconnect immediately, usually retrying the caller's own request on the healed connection (see [Auth failure recovery](./tool-execution#auth-failure-recovery), which covers both cases).
...
| `POST /api/mcp/client/{id}/refresh-tools` | ✅ Re-lists over the live connection | ✅ Ephemeral connect, `tools/list`, disconnect | ✅ Same, via the retained admin discovery credential |
...
`refresh-tools` is the exception to that table's shape: it is the one endpoint that applies to every combination. Where the others recover a *connection* or a *credential* — and so only mean something for the modes that hold one — it re-discovers the *tool list*, which every client has regardless of how it connects. Reach for it when an upstream server's tools changed and you don't want to wait out the [tool sync interval](./connecting-to-servers#global-tool-manager-settings) (10 minutes by default). It returns `400` when the client is in a state where discovery is meaningless: `disabled`, `needs_reauth`, or `pending_verification`.
...
- [MCP Gateway](./overview#bifrost-as-an-mcp-gateway) — reconnection behavior, dynamic tool discovery, health monitoring
