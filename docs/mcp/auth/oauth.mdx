...
<Warning>
The `redirect_uri` Bifrost registers with the upstream provider is locked to Bifrost's current public URL (`mcp_external_client_url`, or the request `Host` header if unset). If you change Bifrost's public URL later, the upstream provider will reject the next authorize call with **"Invalid redirect URI"**. Plain reauthorization reuses the registered client, so it cannot fix the mismatch on its own: use [Reauthorize with a new client](#reauthorizing-with-a-new-client) so Bifrost re-runs DCR against the new URL. (For manually registered credentials, add the new redirect URI in the provider's dashboard instead, then [reauthorize](#reauthorization).)
</Warning>
...
### Reauthorizing with a new client

Reauthorization reuses the `client_id` already stored on the OAuth config. That is the right default, but it cannot recover a client whose provider has stopped recognising that `client_id` altogether. Authorization servers that hand out clients through [Dynamic Client Registration](#dynamic-client-registration-rfc-7591) commonly keep the registry in memory, so a provider restart can drop every client it ever issued.

Once that happens the stored `client_id` is rejected everywhere it is used:

- background refresh fails, and the credential lands in `needs_reauth` with a status reason like `provider rejected the refresh (HTTP 401, invalid_client: Invalid client_id)`
- **Reauthorize** fails the same way, because the authorize request and the code exchange behind it both present the same unknown client

The repair is to register a replacement client first. From the dashboard, open the server's actions menu and choose **Reauthorize with a new client**. Bifrost registers a fresh client at the provider's `registration_url`, writes it over the stored credentials, and then runs the normal consent flow against it. The credential block flags this case: when a `needs_reauth` credential's status reason names `invalid_client`, it says so and points at this action.

<Warning>
Replacing the `client_id` invalidates every token issued under the old one, since none of them can be refreshed against a client the provider no longer associates them with. On a shared `oauth` server that is the one connection credential. On a `per_user_oauth` server it is **every end user's credential as well**, and each of them will have to sign in again. Plain **Reauthorize** leaves end users alone; only this action touches them.
</Warning>

The same flow is scriptable via `POST /api/mcp/client/{id}/reregister` (`{id}` = MCP client ID):

```bash
curl -X POST http://localhost:8080/api/mcp/client/mcp_client_abc123/reregister
```

The response is the same `pending_oauth` payload as `reauthorize`, plus `registered_client_id` and `previous_client_id` so you can confirm which client the consent you are about to complete belongs to. Drive it identically: open `authorize_url`, poll `status_url` until `authorized`, then POST `complete_url`.

It is a separate endpoint rather than a flag on `reauthorize` because the two differ in what they destroy, and that difference belongs somewhere access control and audit logs can see it.

Error cases: everything `reauthorize` returns, plus 400 when the OAuth config has no `registration_url` (the provider offers nowhere to register, so only a manually issued `client_id` can be used) or when the provider refuses the registration request (answers it with a 4xx). A provider that cannot be reached, or that answers with a 5xx, is a 500: nothing in the request was wrong, and nothing was changed, so it is safe to retry.

<Warning>
One 500 is not safe to retry. The replacement client is registered and stored **before** the consent flow is opened, and a registration made at the provider cannot be undone. If opening the flow then fails, the new `client_id` is already installed and every token bound to the config is already `needs_reauth`. The error message says so and names both `client_id`s.

Do not call `reregister` again from that state, because it would register yet another client. Call [`reauthorize`](#reauthorization) instead: it runs consent against whatever client is stored, which is now the replacement.
</Warning>
...
You changed Bifrost's public URL after the upstream client was registered. Use [Reauthorize with a new client](#reauthorizing-with-a-new-client) so Bifrost re-runs DCR with the new URL; for manually registered credentials, add the new redirect URI at the provider's dashboard and then [reauthorize](#reauthorization).
</Accordion>

<Accordion title="`invalid_client: Invalid client_id` on refresh, and Reauthorize does not help">

The provider has stopped recognising the `client_id` itself, not just the token issued under it, so redoing consent with the same client fails at the code exchange the same way the refresh did. This is common with Dynamic Client Registration, where the provider's client registry is often in memory and does not survive a restart.

Use [Reauthorize with a new client](#reauthorizing-with-a-new-client) to register a replacement client and run consent against it. Note that this signs out every credential bound to the server, including every end user's on a `per_user_oauth` server.

If the provider has no `registration_url`, register a client in its dashboard by hand, set the new `client_id` / `client_secret` on the MCP client, and then [reauthorize](#reauthorization).
</Accordion>
</AccordionGroup>
...
| `/api/mcp/client/{id}/reregister` | POST | Register a replacement OAuth client, then redo consent against it, for a provider that no longer recognises the `client_id` it issued (`{id}` = MCP client ID); same payload plus `registered_client_id` / `previous_client_id` |
