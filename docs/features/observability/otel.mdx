...
`export_raw_payloads` attaches the **raw bodies Bifrost exchanged with the provider** to LLM spans, as `bifrost.raw_request` and `bifrost.raw_response`. This is what makes the provider's exact wire format visible in Langfuse or any other OTLP backend.
...
<Warning>
Raw payloads are the least redacted data Bifrost holds. Normalized message content passes through redaction; raw bodies are what the provider actually received and returned, including anything a plugin rewrote. Enabling this sends the provider's exact payload to your collector. Bodies over 256 KB are dropped rather than truncated.
</Warning>
