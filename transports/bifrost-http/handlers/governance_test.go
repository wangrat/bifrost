...
func TestValidateRoutingFallbacks(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "object with pinned key", fbsJSON: `[{"model":"azure/gpt-4o","key_id":"k1"}]`, wantErr: false},
		{name: "object pinning a key for the incoming model", fbsJSON: `[{"model":"azure/","key_id":"k1"}]`, wantErr: false},
		{name: "key_id without a provider prefix rejected", fbsJSON: `[{"key_id":"k1"}]`, wantErr: true},
		{name: "provider_key_name rejected over the API", fbsJSON: `[{"model":"azure/","provider_key_name":"prod"}]`, wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
