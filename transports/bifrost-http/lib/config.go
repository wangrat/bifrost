	...
	return config.ConfigStore.ExecuteTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i := range governanceConfig.RoutingRules {
			...
			for j := range governanceConfig.RoutingRules[i].ParsedFallbacks {
				...
				fallbackProvider, _ := fallback.Split()
				if fallbackProvider == "" {
					return fmt.Errorf("routing rule %q fallback provider_key_name requires model to carry a known provider prefix", governanceConfig.RoutingRules[i].ID)
				}

				keyID, err := resolveProviderKeyIDByProviderAndName(string(fallbackProvider), keyName)
				if err != nil {
