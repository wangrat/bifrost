...
export interface RoutingRuleConfig {
  ...
  fallbacks?: Array<string | { model: string; key_id?: string }>
}
...
export class RoutingRulesPage extends BasePage {
  ...
  async createRoutingRule(config: RoutingRuleConfig): Promise<void> {
    ...
    for (const fallback of config.fallbacks ?? []) {
      // The wire form is combined; the sheet drives provider and model as separate selects.
      const combined = typeof fallback === 'string' ? fallback : fallback.model
      const keyId = typeof fallback === 'string' ? undefined : fallback.key_id
      const separator = combined.indexOf('/')
      const provider = separator === -1 ? combined : combined.slice(0, separator)
      const model = separator === -1 ? undefined : combined.slice(separator + 1) || undefined

      const index = await this.addFallbackProvider(provider, model)
      if (keyId) {
        await this.pinFallbackKey(index, keyId)
      }
    }
