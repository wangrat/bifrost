...
// An event type Bifrost cannot model yields zero chunks from the converter; the
// provider loop then synthesizes a ProviderRawEvent carrier (passthrough only).
// That synthetic type must be invisible on every typed surface.
func TestProviderRawEventFilteredOnTypedSurfaces(t *testing.T) {
	var event AnthropicStreamEvent
	if err := sonic.Unmarshal([]byte(`{"type":"safeguards_update","safeguard_results":[{"id":"sg_1"}]}`), &event); err != nil {
